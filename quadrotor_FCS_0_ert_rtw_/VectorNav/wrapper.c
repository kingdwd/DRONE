...
void InitMicroHard()
{
	...
	gcAddr.sin_port = htons(14550);
        Create_packets(&quadrotor_FCS_0_U,sock);
        close(sock);
...
void GetSerialData(ExtU_quadrotor_FCS_0_T *data)
{
	...
	{
	 char *unconverted;
	 data->RC[i] = strtod(token, &unconverted);
	 i++;
	}
...
*/
	...
	data->RC[1] = (data->RC[1]-950)/1200;
	data->RC[2] = (data->RC[2]-950)/1200;
	data->RC[3] = (data->RC[3]-950)/1200;
	data->RC[4] = (data->RC[4]-950)/1200;
	PDEBUG(0, "%s", buf);
	printf("\n pwm are %f %f %f %f\n",data->RC[1],data->RC[2],data->RC[3], data->RC[4]);
...
{char buf3[128];
 int len;
	...
	char *token = NULL;
	int i = 0;
 WriteLine(tty_fd, "GETOP\n");
 while(!ReadLine(tty_fd, buf3));
 for(token = strtok(buf3, delim); token != NULL; token = strtok(NULL, delim))
	{
	 char *unconverted;
	 data->ORC[i] = strtod(token, &unconverted);
	 i++;
	}
