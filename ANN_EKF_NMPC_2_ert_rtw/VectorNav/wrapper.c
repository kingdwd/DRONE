...
void GetIMUData(ExtU_ANN_EKF_NMPC_2_T *data)
{
	...
	data->GPSVelocitymeas.V_down = Velocity.c2;
        ...
        data->EulerAnglesmeas.phi= ypr.c2*0.0174532925;   // degree to rad[for our matlab model]
	data->EulerAnglesmeas.theta= ypr.c1*0.0174532925;
	data->EulerAnglesmeas.psi= ypr.c0*0.0174532925;
...
void GetSerialData(ExtU_ANN_EKF_NMPC_2_T *data)
{
        ...
        for(token = strtok(buf, delim); token != NULL; token = strtok(NULL, delim))
	{
	 ...
	 {
	 case 0:
	 	data->PICCIC = strtod(token, &unconverted);
		break;
	 case 1:
	 	data->PICCIC = strtod(token, &unconverted);
		break;
...
void InitStaticWaypoints(ExtU_ANN_EKF_NMPC_2_T *data)
{
	...
	data->lat[0]	       = 38.915422;   //reference waypoints in clinton lake
	data->lon[0]	       = -95.317747;
	data->alt[0]	       = 0.3048 * 838;	
}
...
/*******		waypoint conversion from degress to feet	***********/
void latlon(ExtU_ANN_EKF_NMPC_2_T *data, int i)
{
	float fact_lat = 364574.1647;  //factor for converting from deg to feet, pointing to clinton lake
	float fact_lon = 283665.7711;
	data->WaypointsIN.n[i-1] = (data->lat[i] - data->lat[0])*(fact_lat);
	data->WaypointsIN.e[i-1] = (data->lon[i] - data->lon[0])*(fact_lon);
	data->WaypointsIN.h[i-1] = (data->alt[i] - data->alt[0])*(1/.3048);	
}

/*******		end of waypoint conversion			***********/
